...
fn register_memory_event_v2(containere_id:&str, cg_dir: String, memory_event_name:&str, cgroup_event_name:&str) -> Result<Receiver<String>>{
    ...
    info!(sl!(), "register_memory_event_v2 event_control_path: {:?}", &event_control_path);
    info!(sl!(), "register_memory_event_v2 cgroup_event_control_path: {:?}", &cgroup_event_control_path);

    let fd = Inotify::init(InitFlags::empty()).unwrap();

    // watching oom kill
    let ev_fd = fd.add_watch(&event_control_path, AddWatchFlags::IN_MODIFY).unwrap();
    // Because no `unix.IN_DELETE|unix.IN_DELETE_SELF` event for cgroup file system, so watching all process exited
    let cg_fd = fd.add_watch(&cgroup_event_control_path, AddWatchFlags::IN_MODIFY).unwrap();
    info!(sl!(), "ev_fd: {:?}", ev_fd);
    info!(sl!(), "cg_fd: {:?}", cg_fd);
    ...
    thread::spawn(move || {
        loop {
            ...
            info!(sl!(), "container[{}] get events for container: {:?}", &containere_id, &events);
