...
fn load_kernel_module(
    ...
    info!(sl!(), "load_kernel_module {}: {:?}", module.name, module.parameters);
    ...
    match status.code() {
        Some(code) => {
            ...
            let msg = format!("load_kernel_module return code: {} stdout:{} stderr:{}", code, std_out, std_err);
            return Err(ErrorKind::ErrorCode(msg).into());
        },
        None       => return Err(ErrorKind::ErrorCode("Process terminated by signal".to_string()).into()),
    }
}
