...
func initLog() {
	magentLog := logrus.WithFields(logrus.Fields{
		"name": "magent",
		"pid":  os.Getpid(),
	})
	...
	magentLog.Logger.SetLevel(level)
	magentLog.Logger.Formatter = &logrus.TextFormatter{TimestampFormat: time.RFC3339Nano}

	magent.SetLogger(magentLog)
}
