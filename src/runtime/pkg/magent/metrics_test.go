...
func TestEncodeMetricFamily(t *testing.T) {
	...
	for _, line := range lines {
		...
		// only check kata_magent_running_shim_count and kata_magent_scrape_count
		if fields[0] == "kata_magent_running_shim_count" {
			assert.Equal("11", fields[1], "kata_magent_running_shim_count should be 11")
		} else if fields[0] == "kata_magent_scrape_count" {
			assert.Equal("2", fields[1], "kata_magent_scrape_count should be 2")
		}
	}
}
