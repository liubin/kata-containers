...
func (km *KataMonitor) aggregateSandboxMetrics(encoder expfmt.Encoder) error {
	...
	for sandboxID, runtime := range sandboxes {
		...
		go func(sandboxID, namespace string, results chan<- []*dto.MetricFamily) {
			sandboxMetrics, err := km.getSandboxMetrics(sandboxID, runtime)
			if err != nil {
...
func (km *KataMonitor) getSandboxMetrics(sandboxID, runtime string) ([]*dto.MetricFamily, error) {
	body, err := km.doGet(sandboxID, runtime, defaultTimeout, "metrics")
	if err != nil {
		return nil, err
	}
