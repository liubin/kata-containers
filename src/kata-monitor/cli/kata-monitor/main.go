...
import (
	...
	kataMonitor "github.com/kata-containers/kata-containers/src/kata-monitor/pkg/kata-monitor"
...
var (
	...
	monitorListenAddr = flag.String("listen-address", ":8090", "The address to listen on for HTTP requests.")
	runtimeEndpoint   = flag.String("runtime-endpoint", "/run/containerd/containerd.sock", `Endpoint of CRI container runtime service. (default: "/run/containerd/containerd.sock")`)
	logLevel          = flag.String("log-level", "info", "Log level of logrus(trace/debug/info/warn/error/fatal/panic).")
)
